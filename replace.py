...
def replace(args):
    ...
    n_correct_random_random_word = 0
    for batch_idx, batch in enumerate(iterator):
        ...
        batch_text_random_random_word = batch.text.data.clone().transpose(0, 1)
        for i in range(batch.batch_size):
            ...
            for j in rnd_order[:args.n_replace]:
                ...
                repl = random.randint(0, len(TEXT.vocab) - 1)
                batch_text_random_random_word[i][j] = repl
        ...
        y = model(Variable(batch_text_random_random_word).transpose(0, 1))
        preds.append(torch.max(y, 1)[1].view(batch.label.size()))
        n_correct_random_random_word += (preds[5].data == batch.label.data).sum()
        ...
        for i in range(batch.batch_size):
            ...
            checkpoint[-1].append((to_sentence(batch_text_random_random_word[i]),
                                   LABEL.vocab.itos[preds[5][i].item()]))

    with open('replace.pkl', 'wb') as f:
    ...
    print(n_correct_random_random_word / n_total * 100)
